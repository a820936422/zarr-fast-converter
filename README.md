...
v1.6.2 会在生成重采样计划和正式执行前导入 xESMF/ESMF，检查 `ESMFMKFILE`、当前
Pixi 前缀和 `libesmf_fullylinked.so` 是否一致；环境复制后残留旧绝对路径时会在创建
中间 Zarr 前停止，并给出 `pixi reinstall esmf esmpy --locked` 修复命令。一条龙任务
开始时还会打印实际临时根目录、转换中间 Zarr、最终输出目录、预计临时峰值和两个文件系统
的可用空间；空间不足时在正式转换前停止。失败 manifest 记录 `failed_stage`，已验证的中间
产物继续保留用于排查。数值替换、显式 codec/等级选择和任务磁盘监控保持可用。
