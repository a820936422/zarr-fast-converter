...
一条龙模块会先执行完整的数据检查和时间规则确认，然后按统一计划依次完成源读取范围
裁剪转换、xESMF 重采样、最终重分块和无损压缩。转换阶段的源窗口会根据目标边界、源/目标
...
数据检查成功后可以保存完整检查快照，之后通过“导入检查快照”恢复逐文件时间索引和变量结构，直接进入转换；旧版仅含摘要的快照会要求重新检查。
