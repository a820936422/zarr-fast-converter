...
| `src/fast_nc_zarr/runtime.py` | 进程调度 | `bounded_process_map()`、`configure_process_runtime()`、`parse_cpu_affinity()`、`apply_cpu_affinity()` | 有界 pending、线程环境约束、CPU affinity | `FAST_NC_ZARR_THREADS_PER_WORKER=1`；`FAST_NC_ZARR_CPU_AFFINITY` |
