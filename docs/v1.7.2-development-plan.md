...
- [x] Rust `TaskRequest`、`TaskEvent`、`TaskSummary` 和 `TaskStatus` 类型。
- [x] Python pipeline task 使用 dedicated worker，多个任务不再共享同一 worker 锁。
- [x] Tauri 实时转发 accepted、started、resource、progress、log 和 terminal events。
- [x] Rust resource snapshot 进入 task summary 和统一 event envelope。
- [x] 每个 task 拥有独立 cancellation handle，已结束任务不能再次写入取消文件。
- [x] worker 启动失败、协议失败和无 terminal event 统一为结构化 failed event。
- [x] `start_native_task` 不启动 Python worker，首批支持 `zarr.inspect` 和 `zarr.rechunk_f32`。
- [x] native 重分块透传 progress、取消和失败语义，目标目录发布前拒绝覆盖已有目录。
- [x] IPC contract 增加 `native_task`，protocol version 仍保持 `1`。

P1 验收：Tauri package `cargo test` 为 `10 passed`；Python worker/contract targeted
tests 为 `7 passed`；TypeScript typecheck 通过。默认 pipeline 仍使用 Python backend，native
capability 只在明确的 Rust command/task 和 `backend=auto`/`backend=rust` 请求中启用。

### P2 基础 parity 已落地（2026-08-13）

- [x] React inspection 流程覆盖输入路径、输入类型、递归、engine、时间规则和结构结果。
- [x] React pipeline 页面增加 output、变量、resampling、rechunk、recompress 和 backend 设置。
- [x] React task center 展示 task 状态、资源快照、manifest、实时事件和取消入口。
- [x] React 增加 capability 状态、Rust native Zarr inspection 入口、路径收藏和最近目录。
- [x] 复杂格式、复杂重采样和默认 Python reference 语义继续保留。

P2 后续仍需补齐完整 snapshot/恢复页面、真实多变量 native parity 和跨平台 UI 发布验收。

### P3 当前进度（2026-08-13）

- [x] Rust/PyO3 增加 float64 Zarr chunk、region 和 array write API。
- [x] Rust/PyO3 增加单变量三维 `zarr.rechunk_f64`，支持有界并行、取消、progress 和原子 staging publish。
- [x] `zarr.rechunk_f64` 保留源 codec；新的 float64 codec candidate 仍明确 fallback Python。
- [x] capability matrix 和 golden fixture 同步增加 float64 operation IDs。
- [x] Python native adapter 按 dtype 选择 `rechunk_f32_json`/`rechunk_f64_json`，保留 `auto` fallback。
- [x] float64 Python↔Rust fixture 覆盖读写、dtype、chunk/region 和 lossless rechunk。

当前仍使用 Python fallback：多变量 native publish（通过 `zarr.rechunk_multi` capability
明确记录）、整数 dtype、coordinate/data-variable 语义扩展、fill/scale/CF 属性转换、自动压缩
候选和复杂 codec。P3 native smoke/contract targeted tests 当前为 `23 passed`。

### P4/P5 决策门

标准 NetCDF native conversion 和规则网格 native resampling 尚未进入默认候选：当前没有
通过真实数据、metadata/fill 语义、取消/异常安全和跨平台 ABI 验证的实现。HDF-EOS、TIFF、
非标准 calendar、xESMF conservative/patch 和复杂缺测继续由 Python reference 负责；不以
声明 capability 代替正确性门。
...
读写和单变量 `float32`/`float64` 重分块；Tauri P1 runtime 可以在不启动 Python
worker 的情况下执行已声明的 native operation。React pipeline 现在默认发送 `backend=auto`：
复杂源数据转换仍由 Python worker 负责，最终化阶段按 capability 选择 Rust 或记录 Python fallback。
