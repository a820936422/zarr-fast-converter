...
当前仍使用 Python fallback：多变量 native publish、整数 dtype、coordinate/data-variable
语义扩展、fill/scale/CF 属性转换、自动压缩候选和复杂 codec。P3 native smoke/contract
targeted tests 当前为 `21 passed`。
...
读写和单变量 `float32` 重分块/重压缩；Tauri P1 runtime 现在可以在不启动 Python
worker 的情况下执行 `zarr.inspect` 和 `zarr.rechunk_f32`，并统一管理 task、取消、资源
快照和事件。React pipeline 请求仍固定使用 Python backend，等待后续 parity 和决策门。
