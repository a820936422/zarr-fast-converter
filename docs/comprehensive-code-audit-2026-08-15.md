# Fast NC Zarr v1.7.3 基线全面代码功能审查报告

- **审查日期**：2026-08-15
- **审查对象**：`/run/media/owen/HDD/zarr-fast-converter-v1`
- **项目定位**：面向个人用户的 NetCDF/HDF/TIFF 等遥感数据到 Zarr v3 的转换、重采样、重分块、重压缩和 Tauri 桌面处理平台
- **审查方式**：4 个只读审查子代理，分别覆盖 Rust 核心、Python 数据处理链、Tauri/桌面安全、质量与发布运维；均通过 `omp --model yuanheng/gpt-5.6-luna` 显式路由到 **gpt-5.6-luna**。主审查过程补充了关键源码复核、行为复现和定向测试。本文件记录 v1.7.3 基线，不替代 v1.7.4 实施状态。

## 1. 结论摘要

当前代码已经具备较完整的工程骨架：Python 兼容处理链、Rust native 能力、PyO3 扩展、Tauri + React 桌面端、JSONL worker 协议、任务恢复、staging 发布和较多定向测试均已存在。Python 重分块/重采样/发布路径的工程化程度明显高于 native NetCDF 转换和桌面 native 任务边界。

**不建议当前版本直接作为“数据正确性保证”的稳定发布版。** 主要阻断原因不是缺少功能，而是以下四类高风险问题同时存在：

1. **科学数据语义风险**：native NetCDF 转换未一致处理 `_FillValue`、`scale_factor`、`add_offset`；native nearest 对越界坐标返回边缘值；TIFF 序列未校验 CRS 一致性；非标准 calendar 可能被强制转为 Gregorian 日期。
2. **发布与任务状态风险**：native NetCDF 直接写最终目录，失败可留下部分 Zarr；取消检查与发布顺序存在竞态，任务可能报告 `cancelled` 或 `finished`，但输出状态相反。
3. **桌面边界与资源风险**：worker 的 `cancel_task` 对正在执行的同步请求无效；JSONL stdout 可能被检查阶段的人类可读输出污染；native resample 通过 Python list/JSON 往返，峰值内存可严重放大；任务、payload、历史和 request ID 缺少统一上限。
4. **能力与发布契约漂移**：能力矩阵声明的操作多于 Tauri dispatcher 实际支持的操作；inspection 对整数 NetCDF 的支持判断与 conversion 不一致；Python wheel 未声明运行时依赖；CI 未执行桌面 Rust 测试和安装后启动/IPC smoke。
...
- **发布模型**：Python 的多个路径使用 staging + validation + atomic publish；native NetCDF conversion 和部分 native task 目前未统一遵守该边界。

### 2.2 关键边界

当前最重要的边界是：前端只调用 Tauri commands；Tauri 将复杂处理转给 Python worker，同时对部分操作走 Rust native；能力矩阵由 model crate 提供，但桌面 dispatcher 又维护了一套独立的 `NATIVE_OPERATIONS`。这两套清单目前不是单一事实源，导致路由和 UI 能力显示不可靠。
...
**H-06 — TIFF filename mode 未校验每个文件 CRS/投影一致性。**

- **证据**：`src/fast_nc_zarr/filename_mode.py:707-761` 的 rasterio signature 只含 dtype、形状、lat/lon 轴和 fill/scale attrs，不含 CRS/WKT/transform identity；`779-800` 只比较变量签名和轴 hash；输出在 `:1426-1435` 保留首文件 scalar CRS 坐标。
- **影响**：数值像素网格相同但 CRS 不同的文件会被接受，最终 Zarr 被错误标注为首文件 CRS。
- **建议**：signature 加入规范化 CRS、transform、axis direction、nodata 和 band policy；不一致时拒绝，除非显式选择 reprojection 策略；加入真实 GeoTIFF fixtures。
...
**H-09 — 取消机制可能报告错误终态，甚至“已完成但输出已被删除”。**
...
**H-10 — Python worker 的 JSONL stdout 可能被人类可读 inspection 输出污染。**
...
**H-11 — `cancel_task` 无法取消同一 worker 正在执行的请求。**
...
**H-12 — semantic constraints 只是 warning，且 direct-final 路径先发布后校验。**
...
**H-14 — Dask fallback 重命名变量后未清理 CF references。**
...
**H-17 — Python package wheel 未声明运行时依赖。**

- **证据**：`pyproject.toml:5-10` 没有 `[project].dependencies`；源代码在 `src/fast_nc_zarr/cli.py:8-20`、`rechunking/engine.py:15-19`、`resampling/engine.py:14-18` 直接依赖 numpy/xarray/zarr/dask/raster/NetCDF 等；依赖只在 `pixi.toml:8-31` 声明。
- **影响**：在干净 Python 3.13 环境安装 wheel 后，导入或运行命令可能失败，且失败时机远离安装阶段。
- **建议**：把核心依赖写入 project metadata，HDF/TIFF/xESMF 等拆成 optional extras；增加干净环境 wheel install/import/CLI smoke。若产品刻意只支持 Pixi，应删除或明确限制 wheel 发布承诺。
...
**M-09 — `csp: null` 缺少 WebView 防御纵深。**（当前未发现可利用 XSS sink）

- **证据**：`apps/desktop/src-tauri/tauri.conf.json:22-24` 设置 CSP 为 null；命令 handler 在 `lib.rs:33-48` 暴露文件/任务相关能力。当前 frontend 未发现 `innerHTML`、`eval` 或远程 origin。
- **风险**：未来引入远程内容或 XSS sink 后，脚本可调用强权限 Tauri commands。
- **建议**：production 使用 restrictive CSP，禁止远程 origin，并把高权限 command 按 window/capability 进一步分隔。

**M-10 — release package 没有签名或 sidecar integrity verification。**

- **证据**：CI 使用 `scripts/build_desktop.sh --ci --no-sign`（`.github/workflows/native-preparation.yml:92-95`）；`tauri.conf.json:26-30` 打包 external sidecar；CI 只检查包数量、metadata 和 worker 路径，不验证 digest/provenance。
- **影响**：被篡改的安装包或 sidecar 可在用户启动时执行任意代码，属于发布供应链风险。
- **建议**：签名 deb/rpm 并发布签名；构建时记录并验证 sidecar digest、版本和来源；缺少新 sidecar 时构建失败，不允许沿用 stale binary。

**M-11 — worker stderr 被丢弃，启动和 import 失败缺少诊断。**

- **证据**：协议文档要求 stderr 提供人类可读诊断（`contracts/README.md:7-10`），但 `apps/desktop/src-tauri/src/worker.rs:57-60` 使用 `Stdio::null()`。
- **影响**：sidecar 缺依赖、导入错误、启动 traceback 最终只呈现泛化错误，难以支持个人用户排障。
- **建议**：专用 reader 采集有界 stderr，写入 task-scoped diagnostic log，并将最后若干行安全地加入 startup/protocol error。

**M-12 — sidecar 脚本可能在没有生成新 worker 时成功退出。**

- **证据**：`scripts/build_desktop_sidecar.sh:22-39` 在 PyInstaller 不存在时可跳过；`tauri.conf.json:26-30` 又要求 external worker；脚本未在条件构建前清理 stale binary（`:14,41-46`）。
- **影响**：后续 Tauri build 可能失败，或更危险地把旧 worker 打进新包。
- **建议**：发布模式缺 PyInstaller 必须 fail；清理/隔离 sidecar 输出，校验生成 artifact 的 digest、版本和 mtime。

**M-13 — release-candidate 的 Cargo 输出路径未带 target triple。**

- **证据**：`pixi.toml:59` 从 `target/$profile/bundle` 复制；CI 构建使用 `target/x86_64-unknown-linux-gnu/release/bundle`（workflow `:95,100-145`）。
- **影响**：本地或 CI release-candidate 可能找不到刚构建的包，或者复制错误目录中的旧产物。
- **建议**：统一 target/profile 参数和 bundle root 计算，增加“构建后复制的包 hash 与刚生成包一致”的检查。

### 3.6 Medium/Low：测试、契约、文档和隐私

**M-14 — contract 版本和 fixture 仍是 1.7.2，版本检查未覆盖它们。**

- **证据**：`VERSION`、runtime manifests 为 1.7.3；`contracts/README.md:1,90` 和 `contracts/fixtures/capability-v1.json:3-4` 仍为 1.7.2；`scripts/check_version_consistency.py:48-59` 未检查这些文件。
- **影响**：发布标签与 IPC/capability 证据不一致，客户端可能按旧契约解析。
- **建议**：把 contract fixtures 和版本化文档加入一致性检查，或移除其中不必要的 release version。
...
**M-16 — 桌面 CI 未测试 Tauri Rust runtime 和安装后应用。**

- **证据**：`pixi.toml:47-49` 的 core Rust test 排除 desktop；workflow `:34-48` 没有 `cargo test -p fast-nc-zarr-desktop`；desktop job 只检查 frontend build、`tauri info`、包数量/metadata/worker path（`:78-145`），没有启动 packaged binary 或执行 IPC task。
- **影响**：包可成功构建但启动、worker、资源、字体、命令或 X11/Wayland 运行时失败。
- **建议**：CI 加入 desktop Rust tests、headless packaged launch、至少一次 inspection/task IPC smoke，以及字体/icon/frontend asset 的实际加载检查。
...
**L-02 — 文档链接引用不存在的开发方案文件。**

- **证据**：根 `README.md:154-157` 和 `docs/README.md:27-30` 链接 `docs/v1.7.3-development-plan.md`，当前 `docs/` 目录没有该文件。
- **影响**：贡献者按文档导航时得到 404/broken path，架构边界和发布前置条件难以追溯。
- **建议**：恢复文档，或删除/改为实际存在的架构与审查文档链接。
...
- 版本一致性脚本对已覆盖的 runtime/build manifests 检查通过，当前版本为 1.7.3。
...
使用项目 Pixi 环境和当前 native extension 复现：

1. **整数 NetCDF 契约漂移**：inspection 返回 `supported_subset=True`；conversion 返回 `RuntimeError: native conversion supports float32/float64...`。
2. **nearest 越界错误**：源 lat/lon 为 `[0,1]`，目标为 `(2,2)`，native nearest 返回 `4.0`，不是 capability/README 声明的 `NaN`。

这些结果说明问题不是只存在于静态审查中；至少 H-03 和 H-08 在当前构建中可直接观察。

## 6. 修复优先级与发布门槛

### P0：在任何面向用户的稳定发布前完成

1. native NetCDF conversion 改为 staging + validation + atomic publish，并补全失败清理。
2. 修复 `_FillValue`/packed scale/offset 的定义和实现，增加端到端数据语义断言。
3. 修复 nearest 越界行为，并将 capability fixture、实现和测试绑定。
4. 禁止外部 dtype 进入 `unreachable!()`；panic 必须转为 terminal failure。
5. 统一 cancellation/commit state，禁止“已发布但 cancelled”和“输出已删但 finished”。
6. 让 inspection capability 与实际 conversion dtype 完全一致。

### P1：下一个迭代完成

1. 消除 JSON/list native resample 的全量 materialization，加入内存/输出配额。
2. 修复 worker stdout 污染和 `cancel_task` 同步阻塞；建立 task registry cancellation 路由。
3. 校验 TIFF CRS/transform，明确非标准 calendar 边界，修复 Dask CF reference sanitization。
4. 拒绝 source/target 嵌套与 alias，修复 sidecar trust root、digest/signature 和 release fail-closed。
5. 将 capability matrix 和 dispatcher 收敛为单一 enum/事实源。
6. 在 `pyproject.toml` 声明 wheel 运行时依赖，或明确取消 wheel 运行承诺。

### P2：发布工程与长期可靠性

1. 为 task、JSONL、数组、维度、输出估算和历史设置上限。
2. 修复 publication/snapshot 的 same-user TOCTOU 和 symlink 临时文件竞态。
3. 增加 desktop runtime/package launch/IPC CI；修复 sidecar stale artifact 和 target path。
4. Schema validate 全部契约实例，覆盖 fixture/version consistency。
5. 加入 TIFF/HDF-EOS/packed/calendar/异常维度真实 fixtures。
6. 脱敏绝对路径并恢复或更新缺失文档链接。
...
- 本次没有执行完整 Python 项目测试套件、完整 release build 或图形界面人工操作；已执行的测试均为定向检查。
